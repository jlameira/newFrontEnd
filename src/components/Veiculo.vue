<template>
    <div>
        ...
        <i id="sonarRastreando" :class="classPm">
         ...
         <simple-dialog :showModal="showModal" :closeAction="closeDialog" :observacao="observacao"></simple-dialog>
...
<!-- 
        ...
        <div id="listaCompartilhar" class="jm c-user no" style="margin-top: 60px;">
            ...
            <div class="rol">
                <div class="su busca">
                    <input id="proc1" class="proc1" type="text" placeholder="Pesquise" v-model="nome" onclick="document.getElementById('limpar').classList.toggle('no')" @keyup="recuperaUsuario()">
                    <a id="limpar" onclick="this.classList.toggle('no');document.getElementById('proc1').value=''" class="fe proc2 no" title="Limpar">&nbsp;</a>
                    <a id="icPesq" onclick="recuperarUsuarioFiltro(document.getElementById('proc1'), 'visualizarMonitoramento')" class="pr proc2" title="Pesquisar">&nbsp;</a>
                </div>
                <div id="dados">
                    <app-link :compartilhado='false' :list='true' v-for="(usuario, index) in listaUsuarios" :image="'https://intranet.policiamilitar.mg.gov.br'+usuario.URL_FOTO" :key=" index " :nome='usuario.NOME_COMPLETO' classPm="list" @botaoLinkAtivado="adicionarUsuarioAComp(usuario) ">

                    </app-link>

                </div>
                <div id="jaAdicionados ">
                    <app-link :compartilhado='true' :list='true' v-for="(usuario, index) in listaUsuarioCompartilhados" :image="'https://intranet.policiamilitar.mg.gov.br'+usuario.URL_FOTO" :key=" index " :nome='usuario.NOME_COMPLETO' classPm="list">
...
export default {
    ...
    methods: {
      ...
      clickAction () {
        ...
        removeCompartilhado(user) {
            ...
            axios.delete('https://localhost.policiamilitar.mg.gov.br/v1/monitoramento/usrcompartilhados/' + dados._id + '/' + user.NUM_POLICIA, dados)
                .then(resp => {
    ...
    mounted() {
        ...
        // this.classPm =''
        // axios.get('https://localhost.policiamilitar.mg.gov.br/v1/monitoramentoveiculo/' + this.$route.params.id)
        //     .then(resp => {
        //         thi.classPm='no'
        //         this.veiculo = resp.data.retorno;
        //         // this.buscaMarca(resp.data.retorno)
